...
import has.annotations.*;
import has.generators.args.Args;
import has.generators.crud.ControllerGenerator;
import has.generators.crud.RepositoryGenerator;
import has.generators.crud.ServiceGenerator;
import com.squareup.javapoet.JavaFile;
import com.squareup.javapoet.TypeSpec;
import has.builders.AnnotationBuilder;
import has.builders.ParameterBuilder;
import has.generators.authentication.AuthenticationConfiguratorGenerator;
import has.generators.authentication.AuthenticationFilterGenerator;
import has.generators.authentication.AuthorizationFilterGenerator;
import has.generators.authentication.UtilGenerator;
import has.generators.authentication.WebConfigGenerator;
...
public class CodeGenerator {
  ...
  private boolean shouldCreateWebConfig(RoundEnvironment roundEnv) {
    ...
    try {
    	...
    	Class<? extends Annotation> webConfigAnnotation = (Class<? extends Annotation>) Class.forName("org.springframework.security.config.annotation.web.configuration.EnableWebSecurity");
    	Class<? extends Annotation> configAnnotation = (Class<? extends Annotation>) Class.forName("org.springframework.context.annotation.Configuration");
    	Class<? extends Annotation> webSecurityConfigAnnotation = (Class<? extends Annotation>) Class.forName("org.springframework.security.config.annotation.web.configuration.WebSecurityConfigurerAdapter");

	    return roundEnv.getElementsAnnotatedWith(webConfigAnnotation).stream()
		    .filter(ewsEl -> ewsEl.getAnnotation(configAnnotation) != null
			    && this.typeUtils.directSupertypes(ewsEl.asType()).contains(this.elementUtils
			    .getTypeElement(webSecurityConfigAnnotation.getCanonicalName()).asType()))
  ...
  private void validateHandlers(TypeMirror success, TypeMirror failure) {
    ...
    try {
	    ...
	    final String authSuccessHandlerName = Class.forName("org.springframework.security.web.authentication.AuthenticationSuccessHandler").getCanonicalName();
    	...
    	throw new RuntimeException("Could not find class org.springframework.security.web.authentication.AuthenticationSuccessHandler");
    }
    ...
    try {
		  ...
		  final String authFailureHandlerName = Class.forName("org.springframework.security.web.authentication.AuthenticationFailureHandler").getCanonicalName();
    	...
    	throw new RuntimeException("Could not find class org.springframework.security.web.authentication.AuthenticationFailureHandler");
    }
  }
